...
def MC(A): 
      ...
      return [ A[0]^A[2]^A[3], A[1]^A[3], A[2], A[3]]
...
def fixed_input_column(input_num):
      ...
      for a in range(list_input[0], 225 * list_input[0] + 1 ):
            for b in range(list_input[1],225 * list_input[1] + 1): 
                  for c in range(list_input[2],225 * list_input[2] + 1):
                        for d in range(list_input[3],225 * list_input[3] + 1):
...
A = np.array(T_col) *3375
def fff(A):
      for i in range(16):
            print('\\texttt{' + str(int(i)) + '}', end = '')
            for j in range(16):
                  print(' & \\texttt{' + str(int(A[i][j])) + '}', end = '')
            print()
...
def fixed_input_state(input_num  ):
      ...
      col_1_num = int(int_str[0] + int_str[4] + int_str[8] + int_str[12], 2)
      col_2_num = int(int_str[1] + int_str[5] + int_str[9] + int_str[13], 2)
      col_3_num = int(int_str[2] + int_str[6] + int_str[10] + int_str[14], 2)
      col_4_num = int(int_str[3] + int_str[7] + int_str[11] + int_str[15], 2)
      ...
      for output_num in range(0,2**16):
            ...
            out_col_1_num = int(output_str[15] + output_str[10] + output_str[6] + output_str[1], 2)
            out_col_2_num = int(output_str[12] + output_str[9] + output_str[5] + output_str[2], 2)
            out_col_3_num = int(output_str[13] + output_str[8] + output_str[4] + output_str[3], 2)
            out_col_4_num = int(output_str[14] + output_str[11] + output_str[7] + output_str[0], 2)
...
np.savetxt('Find_high_pro_CRAFT_8r.txt', Find_high_pro(T ,R, cells ), delimiter=' ')
...
print('T9:')
T = np.dot(T, T0)  
np.savetxt('Find_high_pro_CRAFT_9r.txt', Find_high_pro(T ,R, cells ), delimiter=' ')

print('T10:')
T = np.dot(T, T0)  
np.savetxt('Find_high_pro_CRAFT_10r.txt', Find_high_pro(T ,R, cells ), delimiter=' ')

print('T11:')
T = np.dot(T, T0)  
np.savetxt('Find_high_pro_CRAFT_11r.txt', Find_high_pro(T ,R, cells ), delimiter=' ')

print('T12:')
T = np.dot(T, T0)  
np.savetxt('Find_high_pro_CRAFT_12r.txt', Find_high_pro(T ,R, cells ), delimiter=' ')
